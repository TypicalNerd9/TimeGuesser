...
FName UTimeGuesserGameInstance::GetRandomTimePeriodLevelName(bool Unique) {
	TArray<FName> DataTableRowNames = TimePeriodsDataTable->GetRowNames();
	FName RandomName =  DataTableRowNames[FMath::RandRange(0, DataTableRowNames.Num()-1)];
	if (Unique) {
		if (CurrentLevelName == RandomName) {
			RandomName = GetRandomTimePeriodLevelName(true);
		}
	}
	return RandomName;
}
